...
def UnetSansBN(num_rows: int, num_cols: int, num_channels: int=3, loss_fn=dice_coef_loss) -> Model:
    ...
    up6 = Conv2D(512, 2, padding='same', kernel_initializer='he_normal')(
        UpSampling2D(size=(2,2))(drop5))
    concatenate6 = concatenate([drop4,up6], axis=3)
    conv6 = Conv2D(512, 3, padding='same', kernel_initializer='he_normal')(concatenate6)
    conv6 = Conv2D(512, 3, padding='same', kernel_initializer='he_normal')(conv6)

    up7 = Conv2D(256, 2, padding='same', kernel_initializer='he_normal')(
        UpSampling2D(size=(2,2))(conv6))
    concatenate7 = concatenate([conv3,up7], axis=3)
    conv7 = Conv2D(256, 3, padding='same', kernel_initializer='he_normal')(concatenate7)
    conv7 = Conv2D(256, 3, padding='same', kernel_initializer='he_normal')(conv7)

    up8 = Conv2D(128, 2, padding='same', kernel_initializer='he_normal')(
        UpSampling2D(size=(2,2))(conv7))
    concatenate8 = concatenate([conv2,up8], axis=3)
    conv8 = Conv2D(128, 3, padding='same', kernel_initializer='he_normal')(concatenate8)
    conv8 = Conv2D(128, 3, padding='same', kernel_initializer='he_normal')(conv8)

    up9 = Conv2D(64, 2, padding='same', kernel_initializer='he_normal')(
        UpSampling2D(size=(2,2))(conv8))
    concatenate9 = concatenate([conv1,up9], axis=3)
    conv9 = Conv2D(64, 3, padding='same', kernel_initializer='he_normal')(concatenate9)
